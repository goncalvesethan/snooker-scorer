...
// ── Constants ─────────────────────────────────────────────────────────────
const COLORS = [
	{ id: "yellow", label: "Jaune",  points: 2, ball: "#e8c832", text: "#1a1a1a" },
...
function calcMaxRemaining(reds, colorsOnTable) {
	if (reds > 0) return reds * (1 + 7) + colorsOnTable.reduce((s, c) => s + c.points, 0);
...
function applyShot(state, action) {
	...
	if (action.type === "pot_red") {
		...
		s.consecutivePots += 1;
		const extrasLabel = extras > 0 ? ` (+ ${extras} rouge${extras > 1 ? "s" : ""} bonus)` : "";
		if (s.phase === "endgame") {
			...
			if (s.colorsOnTable.length === 0) {
				...
				s.message = s.winner !== null ? `${s.players[s.winner].name} remporte la frame !` : "Égalité !";
			} else {
		...
		s.missMode = true;
		const contactLabel = action.contact === "nothing"   ? "sans contact"
											 : action.contact === "wrong_red" ? "mauvaise rouge rentrée"
		...
		const nextBallA = s.phase === "endgame" && s.colorsOnTable.length > 0 ? ` → ${s.colorsOnTable[0].label}` : "";
		...
		if (s.redsLeft === 0) {
			s.phase = "endgame";
			const nextBallR = s.colorsOnTable.length > 0 ? ` → ${s.colorsOnTable[0].label}` : "";
		...
		const nextBallF = s.phase === "endgame" && s.colorsOnTable.length > 0 ? ` → ${s.colorsOnTable[0].label}` : "";
		...
		const nextBallE = s.phase === "endgame" && s.colorsOnTable.length > 0 ? ` → ${s.colorsOnTable[0].label}` : "";
	...
	return (
		...
		>
			<span
				...
				style={{ color: ball.text, fontSize: size * 0.30 }}
			>
				{ball.points}
			</span>
		</button>
...
// ── Reusable UI ───────────────────────────────────────────────────────────
function Card({ children, className = "", borderClass = "border border-black/7 dark:border-white/8" }) {
	return (
		<div className={`bg-white dark:bg-slate-900 rounded-[20px] p-[18px_16px] ${borderClass} ${className}`}>
			{children}
		</div>
	);
}
...
function Btn({ children, onClick, disabled, variant = "default", className = "" }) {
	const base = "border rounded-xl px-[10px] py-[11px] font-bold text-[13px] font-[inherit] transition-transform duration-120";
	const states = disabled
		? "opacity-30 cursor-not-allowed"
		: "cursor-pointer hover:-translate-y-px active:scale-[0.97]";
	const variants = {
		default: "bg-white dark:bg-slate-900 border-black/7 dark:border-white/8 text-gray-500 dark:text-slate-300",
		warn:    "bg-orange-500/[0.09] border-orange-500/30 text-amber-600 dark:text-amber-400",
		success: "bg-green-500/[0.09] border-green-500/30 text-green-700 dark:text-green-400",
	};
	return (
		<button
			onClick={onClick}
...
function HistoryLog({ history }) {
	...
	return (
		<div className="mt-4 rounded-2xl overflow-hidden bg-black/3 dark:bg-white/3 border border-black/6 dark:border-white/7">
			...
			<div className="max-h-[500px] overflow-y-auto px-[14px] pb-[10px]">
...
export default function SnookerApp() {
	...
	const startGame = () => {
		setGame(makeInitialState(p1Name.trim() || "Joueur 1", p2Name.trim() || "Joueur 2"));
		setStateHistory([]);
		setSetup(false);
	};

	const dispatch = useCallback((action, logMsg) => {
	...
	const resetGame = () => { setSetup(true); setGame(null); setStateHistory([]); };
	...
	const opp = 1 - cur;
	const nextColor = g.phase === "endgame" && g.colorsOnTable.length > 0 ? g.colorsOnTable[0] : null;
	const canPotRed   = !g.frameOver && !g.missMode && !g.missTargetMode && g.phase === "red" && g.redsLeft > 0;
	const canPotColor = !g.frameOver && !g.missMode && !g.missTargetMode && (g.phase === "color" || g.phase === "endgame");
	const canMiss     = !g.frameOver && !g.missMode && !g.missTargetMode;

	const phaseInfo =
		g.phase === "red"     ? { label: "Jouez une rouge",   dot: "bg-[#e02020] shadow-[0_0_6px_rgba(220,32,32,0.4)]" }
	: g.phase === "color"   ? { label: "Jouez une couleur", dot: "bg-blue-500 shadow-[0_0_6px_rgba(59,130,246,0.4)]" }
	: g.phase === "endgame" ? { label: `Phase finale · ${nextColor?.label || "Fin"}`, dot: "bg-amber-500 shadow-[0_0_6px_rgba(245,158,11,0.4)]" }
	: null;
		...
		<div className="min-h-screen bg-slate-100 dark:bg-[#0e1520] px-4 pt-4 pb-11 text-gray-900 dark:text-slate-100">
			<div className="max-w-[440px] mx-auto">
				...
				<div className="flex items-center justify-between mb-5">
					<div>
						<h1 className="m-0 text-[22px] font-black tracking-[-0.5px]">Snooker</h1>
						<div className="text-[11px] text-gray-400 dark:text-slate-500 mt-[3px] font-semibold">
							...
							<span className="text-blue-600 dark:text-blue-500">
								{calcMaxRemaining(g.redsLeft, g.colorsOnTable)} pts
							</span>
						</div>
					</div>
				...
				{/* ── Phase indicator ── */}
				{phaseInfo && !g.frameOver && !g.missMode && !g.missTargetMode && (
					<div className="text-center mb-3">
						<span className="inline-flex items-center gap-[7px] px-[18px] py-[6px] rounded-full bg-white dark:bg-slate-900 border border-black/7 dark:border-white/8 text-[12px] font-bold text-gray-500 dark:text-slate-400">
							<span className={`w-[7px] h-[7px] rounded-full inline-block animate-pulse ${phaseInfo.dot}`} />
							{phaseInfo.label}
							<span className="opacity-45 text-[11px]">— {g.players[cur].name}</span>
						</span>
					</div>
				)}
				...
				{g.frameOver && (
					<div className="bg-white dark:bg-slate-900 border-2 border-blue-500 rounded-[22px] p-[32px_24px] text-center mb-4 shadow-[0_0_0_6px_rgba(59,130,246,0.22)] animate-fade-up">
						...
						<div className="text-[24px] font-black mb-[6px] tracking-[-0.5px]">
							{g.winner !== null ? `${g.players[g.winner].name} remporte la frame !` : "Égalité !"}
						</div>
						<div className="text-[15px] text-gray-400 dark:text-slate-500 mb-6">
							{g.players[0].name}{" "}
							<strong className="text-gray-900 dark:text-slate-100 text-[18px]">{g.players[0].score}</strong>
							<span className="mx-[10px] opacity-40">—</span>
							<strong className="text-gray-900 dark:text-slate-100 text-[18px]">{g.players[1].score}</strong>{" "}
							{g.players[1].name}
						</div>
						<button
							onClick={resetGame}
				...
				{g.missTargetMode && !g.frameOver && (
					...
					>
						<div className="text-center mb-4">
							<div className="text-[13px] font-extrabold text-amber-600 dark:text-amber-400 mb-1">Miss — type de faute</div>
							<div className="text-[12px] text-gray-400 dark:text-slate-500">
								Comment <strong className="text-gray-900 dark:text-slate-100">{g.players[cur].name}</strong> a-t-il manqué ?
							</div>
						</div>
						...
						<button
							onClick={() => dispatch(
								{ type: "miss_set_target", contact: "nothing", ballPoints: 0, ballLabel: "rien", ballId: "nothing" },
						...
						>
							...
							<span className="text-[12px] text-gray-400 dark:text-slate-500 font-semibold">4 pts</span>
						</button>
						...
						<div className="flex flex-wrap gap-2">
							{[{ id: "red", label: "Rouge", points: 1, ball: "#d42020", text: "#fff" }, ...COLORS].map(ball => {
								const awarded = Math.max(FOUL_MIN, ball.points);
								return (
									<button key={ball.id}
										onClick={() => dispatch(
											{ type: "miss_set_target", contact: "wrong_color", ballPoints: ball.points, ballLabel: ball.label, ballId: ball.id },
										...
										>
										<span className="w-[14px] h-[14px] rounded-full shrink-0" style={{ background: ball.ball }} />
										{ball.label}
										<span className="text-gray-400 dark:text-slate-500 font-semibold">· {awarded}</span>
									</button>
								);
							})}
						</div>
						...
						<div className="text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-[0.1em] mt-[10px] mb-2">
							...
							{g.redsLeft > 0 && (
								<button
									onClick={() => dispatch(
										{ type: "miss_set_target", contact: "wrong_red", ballPoints: 1, ballLabel: "rouge", ballId: "red" },
								...
								>
									<div className="flex items-center gap-[8px]">
										<span className="w-[14px] h-[14px] rounded-full shrink-0" style={{ background: "#d42020" }} />
										<span>Rouge <span className="text-[11px] font-medium opacity-65">(rentrée — retire 1 rouge)</span></span>
									</div>
									<span className="text-[12px] text-red-500 font-bold">4 pts</span>
								</button>
							)}
					</Card>
				)}
				...
				{g.missMode && !g.frameOver && (
					...
					>
						<div className="text-center mb-4">
							...
							<div className="text-[36px] font-black leading-none">{g.missFoulValue}</div>
							<div className="text-[12px] text-gray-400 dark:text-slate-500 mt-1">
								pts attribués à{" "}
								<strong className="text-gray-900 dark:text-slate-100">{g.players[opp].name}</strong>
							</div>
						</div>
				...
				{!g.frameOver && !g.missMode && !g.missTargetMode && (
					<div className="flex flex-col gap-3">
							...
							<Card>
								...
								<div className="flex justify-center">
									<div className="flex flex-col items-center gap-2">
										<RedBall3D
											onClick={() => dispatch({ type: "pot_red" }, `Rouge — ${g.players[cur].name}`)}
											disabled={!canPotRed}
											size={76}
										/>
										<span className="text-[11px] text-gray-400 dark:text-slate-500 font-semibold">Rouge · 1 pt</span>
									</div>
								</div>
							</Card>
						)}
						...
						{(g.phase === "color" || g.phase === "endgame") && (
							<Card>
								...
								{g.phase === "color" && (
									<div className={[
										...
										<div className="flex flex-col gap-0.5">
											<span className={`text-[11px] font-extrabold tracking-[0.08em] uppercase ${g.extraReds > 0 ? "text-[#e02020]" : "text-gray-400 dark:text-slate-500"}`}>
												Rouges empochées
											</span>
										...
										<div className="flex items-center gap-2">
											<div className="flex gap-1 items-center">
												{Array.from({ length: 1 + g.extraReds }).map((_, i) => (
													<div
														key={i}
											...
											<button
												onClick={() => dispatch({ type: "add_extra_red" }, `Rouge bonus (${g.players[cur].name})`)}
								...
								<div className="flex flex-wrap justify-center gap-[14px]">
									{(g.phase === "endgame" ? [g.colorsOnTable[0]].filter(Boolean) : COLORS).map(ball => (
										<div key={ball.id} className="flex flex-col items-center gap-[6px]">
											<Ball3D
												ball={ball}
												size={60}
											...
											/>
											<span className="text-[10px] text-gray-400 dark:text-slate-500 font-semibold">{ball.label}</span>
										</div>
									))}
								</div>
							</Card>
						...
						<div className="grid grid-cols-2 gap-[10px]">
							<Btn
								onClick={() => dispatch({ type: "end_break" }, `➡ ${g.players[cur].name} passe la main`)}
								variant="default"
							>
							Fin de break
							</Btn>
						...
						<Card>
							...
							<div className="flex flex-wrap justify-center gap-[7px]">
								{/* Faute générique */}
								<button
									onClick={() => dispatch({ type: "foul", value: 0 }, `⚠ Faute — ${g.players[opp].name} +4pts`)}
								...
								<button
									onClick={() => dispatch({ type: "foul_red" }, `⚠ Faute rouge rentrée — ${g.players[opp].name} +4pts`)}
									...
									className={[
										...
										g.redsLeft === 0 ? "opacity-30 cursor-not-allowed" : "cursor-pointer hover:-translate-y-px",
									].join(" ")}
								>
									Rouge · 4
								</button>
